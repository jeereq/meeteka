        ...
        <h2 className="text-3xl xs:text-4xl lg:text-5xl font-bold mb-4">
          {t("home.services.title")
            ...
            .map(function (element, index: number) {
              if (index == 0) {
                return (
                  <span className="w-fit" key={index}>
                    {element}
                  </span>
                );
              } else {
                return (
                  <span key={index} className="ml-2 heading-highlight">
                    {" "}
                    {element}
                  </span>
                );
              ...
              <div className="relative">
                ...
                <ul className="space-y-2 mb-6">
                  {service.details.map((detail, i) => (
                    <li
                      key={i}
                      className="flex items-center gap-2 text-sm text-gray-600 group-hover:text-white/80 transition-colors"
                    >
